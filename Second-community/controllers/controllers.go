...
func (uc *UserController) CreateQuestion(c *gin.Context) {
	...
	c.ShouldBindJSON(&q)
	q.UserID = userID
	res, _ := uc.UserService.CreateQuestion(q)
	c.JSON(http.StatusOK, res)
}
...
func (uc *UserController) CreateAnswer(c *gin.Context) {
	...
	c.ShouldBindJSON(&a)
	a.UserID = userID
	res, _ := uc.UserService.CreateAnswer(a)
	c.JSON(http.StatusOK, res)
}

func (uc *UserController) Answer(c *gin.Context) {
	userID := c.MustGet("userID").(uint)
	var a1 models.Answers1
	c.ShouldBindJSON(&a1)
	a1.UserID = userID
	res, _ := uc.UserService.Answer(a1)
	c.JSON(http.StatusOK, res)
}
